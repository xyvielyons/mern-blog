...
try {
    const fetchPost = async ()=>{
        const res = await authFetch.get(`/post/getPosts?postId=${postId}`)
  ...
  const handleSubmit = async (e)=>{
    ...
    try{
      const res = await authFetch.put(`/post/updatepost/${formData._id}/${currentUser._id}`,formData)
      if(res.data.status == "success"){
        navigate(`/post/${res.data.updatePost.slug}`)
      }
    }catch(err){
    ...
    <div className='p-3 max-w-3xl mx-auto min-h-screen'>
      ...
      <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-4 sm:flex-row justify-between'>
          <TextInput value={formData.title} type="text" placeholder="Title" required id="title" className='flex-1' onChange={(e)=>{
